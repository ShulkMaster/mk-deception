...
struct SecArtMember {
    ...
    void* data_or_texture; /* +0x04; tex RwTexture* or binary file offset */
    unsigned int size;     /* +0x08; binary member byte size (STRINGS/SCREEN) */
    char* name_or_data;    /* +0x0C; name string after process_art reloc */
};
...
struct SecSlotFileEntry {
    ...
    int load_state;
    void* buffer;          /* +0x0C; read buffer / SEC base */
