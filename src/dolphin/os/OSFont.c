...
#define OS_FONT_ENCODE_MAX 5
#define OS_FONT_ROM_SIZE_ANSI 0x3000
...
static void Decode(u8* s, u8* d) {
    ...
    do {
        ...
        // Linked chunk
            ...
            j = s[link_index] << 8 | s[link_index + 1];
            link_index += sizeof(u16);

            // Apply offset
            k = q - (j & 0x0FFF);
            cnt    = j >> 12;
            if (cnt == 0) {
                cnt = s[literal_index++] + 0x12;
            } else {
                cnt += 2;
            }
...
static inline void ReadROM(void* buf, int length, int offset) {
    int len;
    while (length > 0) {
