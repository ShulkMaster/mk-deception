...
#define GX_SNAPSHOT_READ_POINTER(fifo) \
do { \
...
#define GX_SNAPSHOT_FIFO_COUNT(fifo) \
do { \
...
static void __GXSaveFifoCPStat(__GXFifoObj* realFifo) {
    ...
    GX_SNAPSHOT_READ_POINTER(realFifo);
    GX_SNAPSHOT_FIFO_COUNT(realFifo);
}
...
inline void GXSaveGPFifo(GXFifoObj* fifo) {
    __GXFifoObj* realFifo = (__GXFifoObj*)fifo;
#if SDK_REVISION < 2
    u32 cpStatus;
    u8 readIdle;
...
inline void __GXInsaneWatermark(void) {
...
inline OSThread* GXGetCurrentGXThread(void) {
