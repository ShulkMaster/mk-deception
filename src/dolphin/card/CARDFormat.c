    ...
    if (result >= 0) {
        ...
        if (card->formatStep < CARD_NUM_SYSTEM_BLOCK) {
            result = __CARDEraseSector(
                chan, (u32)card->sectorSize * card->formatStep,
                FormatCallback);
            if (result >= 0)
                return;
            ...
            card->currentDir =
                (CARDDir*)((u8*)card->workArea + CARD_SYSTEM_BLOCK_SIZE);
            memcpy(card->currentDir,
                   (u8*)card->workArea + 2 * CARD_SYSTEM_BLOCK_SIZE,
                   CARD_SYSTEM_BLOCK_SIZE);
            card->currentFat =
                (u16*)((u8*)card->workArea + 3 * CARD_SYSTEM_BLOCK_SIZE);
            memcpy(card->currentFat,
                   (u8*)card->workArea + 4 * CARD_SYSTEM_BLOCK_SIZE,
                   CARD_SYSTEM_BLOCK_SIZE);
        }
    }
