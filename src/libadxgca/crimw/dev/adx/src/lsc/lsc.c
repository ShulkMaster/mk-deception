...
static inline LSCStreamInfo* lsc_GetWriteEntry(LSCObject* lsc)
{
    return &lsc->stream_info[lsc->write_position];
}

/* TODO: Identify retail's no-inline source control. Keep this scoped guard:
 * object-wide inline modes change otherwise exact sibling functions. */
                       ...
                       void* directory, s32 offset, s32 sector_count)
{
    ...
    stream_id =
        lsc->stream_info[(lsc->write_position + 15) % 16].id;
    info = lsc_GetWriteEntry(lsc);
...
static inline LSCObject* lsc_SearchFreeObj(void)
{
    LSCObject* lsc;
    s32 i;
...
LSCObject* LSC_Create(SJ* sj)
{
    ...
    if (lsc == 0) {
        ...
        lsc->state = 0;
        lsc->buffer_size =
            sj->interface->get_num_data(sj, 0) +
