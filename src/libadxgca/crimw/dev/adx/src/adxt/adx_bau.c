                                 ...
                                 int* codec)
{
    ...
    if (magic != 0x64732E && magic != 0x646E732E)
        return NULL;
    header_size = LE32(header + 4);
    header_size = SWAP32(header_size);
    if (header_size > length)
        return NULL;
    data_size = LE32(header + 8);
    data_size = SWAP32(data_size);
    encoding = LE32(header + 12);
    encoding = SWAP32(encoding);
    switch (encoding) {
    ...
    *rate = LE32(header + 16);
    *rate = SWAP32(*rate);
    *channels = LE32(header + 20);
    *channels = SWAP32(*channels);
    if (*codec == 2)
