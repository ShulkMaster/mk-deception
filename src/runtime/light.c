...
RpLight* create_spot_light(MkObj* parent, LightDef* def) {
    ...
    parent->pos_x = def->field1C;
    parent->pos_y = def->field20;
    parent->pos_z = def->field24;
...
static RpLight* create_type5_spot(MkObj* parent, LightDef* def) {
    ...
    mkobj->pos_x = def->field1C;
    mkobj->pos_y = def->field20;
    mkobj->pos_z = def->field24;
...
MkObj* load_light(LightDef* def, MkPtr** list, MkObj* parent) {
    ...
    case 2:
        ...
        if (parent == 0) {
            mkobj->pos_x = def->field20;
            mkobj->pos_y = def->field24;
            mkobj->pos_z = def->field28;
            insert_fgnd_mkobj(mkobj);
            update_mkobj(mkobj);
        }
        ok = 1;
