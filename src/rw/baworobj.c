...
#define RWASSERTISTYPE(object, type)
...
#define rwLLLinkGetData(link, type, member) \
    ((type*)((RwUInt8*)(link) - (RwUInt32)&((type*)0)->member))
...
#define RPWORLDOBJGLOBAL(var) \
    (RWPLUGINOFFSET(rpWorldObjGlobals, RwEngineInstance, \
                    worldObjModule.globalsOffset).var)
...
AtomicDestroyTies(RpAtomic * atomic)
{
    ...
    end = rwLinkListGetTerminator(&atomic->worldSectorsInAtomic);
    cur = rwLinkListGetFirstLLLink(&atomic->worldSectorsInAtomic);
    while (cur != end)
    {
        /* Remove the tie */
                  ...
                  RwInt32 sizeInObject __RWUNUSED__)
{
    ...
    RWRETURN(_rpMeshWrite(geometry->meshHeader, (const void *) geometry,
                 ...
                 RwInt32 sizeInObject __RWUNUSED__)
{
    ...
    geometry->meshHeader = _rpMeshRead(stream, (const void *) geometry,
                        &geometry->matList);
    if (geometry->meshHeader)
    {
        RWRETURN(stream);
    }
    else
                 ...
                 RwInt32 sizeInObject __RWUNUSED__)
{
    ...
    RWRETURN(_rpMeshSize(geometry->meshHeader, object));
}
