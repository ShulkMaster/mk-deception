...
#define rwRASTERFORMATDEFAULT 0x0000
...
#define rwRASTERFORMAT555 0x0A00
#define rwRASTERFORMATPIXELFORMATMASK 0x0F00
#define rwRASTERFORMATAUTOMIPMAP 0x1000
...
{
    RwDlPixelUnconvertFn result = NULL;
...
{
    RwDlPixelConvertFn result = NULL;
...
static RwImage* _rwDolphinPalettizeImage(RwImage* image, RwInt32 depth)
{
    ...
    if (RwPalQuantInit(&quantizer) == FALSE)
        return NULL;
    ...
    RwPalQuantMatchImage(palettized->pixels, palettized->stride,
                         palettized->depth, FALSE, &quantizer, image);
                               ...
                               RwInt32 unused)
{
    ...
    if ((raster->privateFlags & 4) != 0)
        rasterLocked = TRUE;
    if (rasterLocked == FALSE && RwRasterLock(raster, 0, 5) == NULL)
        return FALSE;

    if ((format & (rwRASTERFORMATPAL4 | rwRASTERFORMATPAL8)) != 0) {
        if ((raster->privateFlags & 0x10) != 0)
            paletteLocked = TRUE;
        if (paletteLocked == FALSE &&
            RwRasterLockPalette(raster, 5) == NULL) {
            return FALSE;
        }
    }
