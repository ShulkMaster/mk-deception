...
#define RWASSERTM(condition, message)
...
enum {
    rwMATRIXTYPENORMAL = 1,
    rwMATRIXTYPEORTHOGONAL = 2,
    rwMATRIXTYPEORTHONORMAL = 3,
    rwMATRIXTYPEMASK = 3,
    rwMATRIXINTERNALIDENTITY = 0x20000,
    rwMATRIXOPTIMIZE_IDENTITY = 0x20000,
    rwID_MATRIX = 0x0D,
    rwMEMHINTDUR_GLOBAL = 0x40000,
    E_RW_BADPARAM = 0x80000003
};

#define rwMATRIXALIGNMENT 16
#define rwPI 3.14159265358979323846
#define rwMatrixGetFlags(matrix) ((matrix)->flags)
#define rwMatrixSetFlags(matrix, value) ((matrix)->flags = (value))
#define rwMatrixTestFlags(matrix, bits) (((matrix)->flags & (bits)) != 0)
#define rwMatrixInitialize(matrix, value) ((matrix)->flags = (value), (matrix))
#define RwMatrixCopyMacro(destination, source) (*(destination) = *(source))
#define RwMatrixCopy(destination, source) RwMatrixCopyMacro(destination, source)
#define RwV3dAssign(destination, source) (*(destination) = *(source))
#define RwRealAbs(value) \
    ((RwReal)((value) >= (RwReal)0.0f ? (value) : -(value)))
#define RwV3dDotProductMacro(a, b) \
    ((a)->x * (b)->x + (a)->y * (b)->y + (a)->z * (b)->z)
#define RwV3dCrossProductMacro(out, a, b) \
    ((out)->x = (a)->y * (b)->z - (a)->z * (b)->y, \
     (out)->y = (a)->z * (b)->x - (a)->x * (b)->z, \
     (out)->z = (a)->x * (b)->y - (a)->y * (b)->x)
#define RwV3dScaleMacro(out, in, scale) \
    ((out)->x = (in)->x * (scale), (out)->y = (in)->y * (scale), \
     (out)->z = (in)->z * (scale))
#define RwV3dSubMacro(out, a, b) \
    ((out)->x = (a)->x - (b)->x, (out)->y = (a)->y - (b)->y, \
     (out)->z = (a)->z - (b)->z)
#define RwV3dIncrementScaledMacro(out, in, scale) \
    ((out)->x += (in)->x * (scale), (out)->y += (in)->y * (scale), \
     (out)->z += (in)->z * (scale))
#define RwV3dLength(vector) \
    (_rwSqrt(RwV3dDotProductMacro((vector), (vector))))
#define _rwV3dNormalizeMacro(result, out, in) \
    do {                                               \
        RwReal lengthSquared = RwV3dDotProductMacro((in), (in)); \
        (result) = _rwInvSqrt(lengthSquared);          \
        RwV3dScaleMacro((out), (in), (result));        \
    } while (0)
#define RwMatrixSetIdentity(matrix)                                 \
    do {                                                            \
        (matrix)->right.x = (matrix)->up.y = (matrix)->at.z = 1.0f; \
        (matrix)->right.y = (matrix)->right.z = (matrix)->up.x =    \
            (matrix)->up.z = (matrix)->at.x = (matrix)->at.y =     \
                (matrix)->pos.x = (matrix)->pos.y =                 \
                    (matrix)->pos.z = 0.0f;                         \
        (matrix)->flags |=                                          \
            rwMATRIXINTERNALIDENTITY | rwMATRIXTYPEORTHONORMAL;     \
    } while (0)
#define rwMat02Det(a, b, c, d) ((a) * (d) - (b) * (c))
#define rwMat03Inv(rAA, rAB, rAC, rBA, rBB, rBC, rCA, rCB, rCC,    \
                   mAA, mAB, mAC, mBA, mBB, mBC, mCA, mCB, mCC)    \
    do {                                                            \
        RwSplitBits determinant;                                    \
        (rAA) = rwMat02Det((mBB), (mBC), (mCB), (mCC));             \
        (rAB) = -rwMat02Det((mAB), (mAC), (mCB), (mCC));            \
        (rAC) = rwMat02Det((mAB), (mAC), (mBB), (mBC));             \
        determinant.nReal =                                         \
            (rAA) * (mAA) + (rAB) * (mBA) + (rAC) * (mCA);         \
        {                                                            \
            const RwReal normalize = determinant.nInt != 0          \
                                         ? 1.0f / determinant.nReal \
                                         : 1.0f;                    \
            (rAA) *= normalize;                                     \
            (rAB) *= normalize;                                     \
            (rAC) *= normalize;                                     \
            (rBA) = -rwMat02Det((mBA), (mBC), (mCA), (mCC)) * normalize; \
            (rBB) = rwMat02Det((mAA), (mAC), (mCA), (mCC)) * normalize; \
            (rBC) = -rwMat02Det((mAA), (mAC), (mBA), (mBC)) * normalize; \
            (rCA) = rwMat02Det((mBA), (mBB), (mCA), (mCB)) * normalize; \
            (rCB) = -rwMat02Det((mAA), (mAB), (mCA), (mCB)) * normalize; \
            (rCC) = rwMat02Det((mAA), (mAB), (mBA), (mBB)) * normalize; \
        }                                                            \
    } while (0)
#define RwSin(value) sinf(value)
#define RwCos(value) cosf(value)
