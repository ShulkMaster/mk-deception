...
#define ALPHA_PASS_DATA(material) \
    ((RpMaterialAlphaPass*)(((unsigned long)(material) + alphaPassPluginOffset + 15) & \
                            ~15UL))
...
static void* AlphaPassConstructor(void* object, int offset, int size) {
    if ((unsigned int)alphaPassPluginOffset != 0) {
        RpMaterialAlphaPass* plugin = ALPHA_PASS_DATA(object);
        plugin->texture = 0;
...
static RwStream* AlphaPassMaterialStreamWrite(RwStream* stream, int length,
                                               ...
                                               int sizeOfObject) {
    int size = 0;
    RpMaterialAlphaPass* plugin = ALPHA_PASS_DATA(material);
    (void)length;
    (void)plugin;
    size += AlphaPassStreamWriteTexture(stream,
...
static RwStream* AlphaPassMaterialStreamRead(RwStream* stream, int length,
    ...
    size += AlphaPassStreamReadTexture(stream, &texture);
    if (texture != 0) {
