...
static void _rwDlVIPreRetraceCallback(void)
{
    ...
    if (_RwDlFrameReadyOnToken == TRUE &&
...
static void _rwDlVIPostRetraceCallback(void)
{
    if (_RwDlFrameWait != FALSE && _RwDlFrameGo != FALSE) {
        _rwDlBreakNext();
                                          ...
                                          RwInt32 numStandards)
{
    ...
    while (numStandardFunctions-- != 0) {
        /* Retail's stock table loop tests the first entry before selecting the
           indexed callback. The first index is positive, so the upper-bound
           test alone controls whether any platform callbacks are installed. */
...
static RwBool _rwDlSystem(RwInt32 option, void* out, void* inOut, RwInt32 in)
{
    ...
    case 19:
        ...
        return TRUE;
    case 3:
        while (_RwDlFrameReadyOnToken == TRUE ||
...
RwBool _rwDlRasterShowRaster(void* out, void* inOut, RwInt32 in)
{
    ...
    if (_RwDlFSAA == 0) {
        ...
        if (_RwDlBreakPointEnabled == FALSE) {
...
void RwGameCubeCameraTextureFlush(RwRaster* raster, RwBool mipmap)
{
    ...
    RwGameCubeRasterExt* extension =
        RW_GAMECUBE_RASTER_EXTENSION(parent);
    RwUInt32 offset;
    ...
    /* Retail lowers this sparse depth switch through a 29-entry jump table.
       Clean typed C selects an equivalent comparison tree with these flags. */
    switch (parent->depth) {
    case 4:
    ...
    GXSetCopyFilter(_RwDlRenderMode->aa, _RwDlRenderMode->sample_pattern,
                    TRUE, _RwDlRenderMode->vfilter);
    if (extension->textureRegion != NULL)
