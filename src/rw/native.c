...
#define NATIVE_MESH_TABLE(native)                                        \
    ((RpGameCubeNativeMesh*)((RwUInt8*)(native) +                       \
        ((native)->numMeshes - 1) * sizeof(RpGameCubeNativeMesh) + 0x14))
#define NATIVE_VERTEX_DATA(meshes, count)                                \
    ((RwUInt8*)(((RwUInt32)(meshes) +                                   \
...
static void _rpNativePointer2Offset(RpGameCubeNativeData* native,
                                    ...
                                    RwUInt32 numMeshes)
{
    RwUInt8* vertexData = NATIVE_VERTEX_DATA(meshes, numMeshes);
    RwUInt32 i;
                           ...
                           RwResEntry** entryRef, RwUInt32 numMeshes)
{
    ...
    if (!RwStreamFindChunk(stream, 1, &chunkLength, &version)) return NULL;
    ...
    if (!RwStreamReadInt32(stream, &platform, 4) || platform != 6)
        return NULL;
    if (!RwStreamReadInt32(stream, &headerSize, 4)) return NULL;
    if (!RwStreamReadInt32(stream, &dataSize, 4)) return NULL;
    ...
    *entryRef = entry;
    ...
    meshes = NATIVE_MESH_TABLE(native);
                                ...
                                RwUInt32 numMeshes)
{
    ...
    RpGameCubeNativeMesh* meshes = NATIVE_MESH_TABLE(native);
    ...
    if (!_rwStreamWriteVersionedChunkHeader(
        ...
        return NULL;
    if (!RwStreamWriteInt32(stream, &platform, 4)) return NULL;

    headerSize = (native->numMeshes - 1) * 8 + numMeshes * 8 + 0x14;
    if (!RwStreamWriteInt32(stream, &headerSize, 4)) return NULL;
    dataSize = entry->size - headerSize - 0x1F;
    if (!RwStreamWriteInt32(stream, &dataSize, 4)) return NULL;

    _rpNativePointer2Offset(native, meshes, numMeshes);
    if (!RwStreamWrite(stream, native, headerSize)) return NULL;
    _rpNativeOffset2Pointer(native, meshes, numMeshes);
    if (!RwStreamWrite(stream, meshes->data, dataSize)) return NULL;
...
{
    if ((geometry->flags & 0x01000000) && geometry->repEntry != NULL)
