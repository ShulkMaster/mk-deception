...
/* Retail selects helper-based saves in the position/texture size leaves even
 * though their clean bodies contain no calls. The sizing CFG and field widths
 * below are otherwise identical; no dead call is retained to force that form.
 */
...
/* The conversion routines retain retail's per-format cursor lifetimes and
 * standalone switch bodies. Remaining differences are SDK conversion-macro
 * temporaries (redundant destination aliases and explicit byte/halfword sign
 * normalization) plus nonvolatile coloring; calls, branches, widths, scaling,
 * strides, and returned byte counts match the retail instruction streams. */
