...
| 26 | If every effective nested-field address agrees but retail keeps `owner + child_offset` live while ours materializes `&owner->child`, keep a typed pointer to the owning aggregate and spell accesses through `owner->child.field` | Struct offsets, access widths, and effective addresses are already exact across multiple accesses; the owner is the real subsystem object | Nested-owner lifetime and base/displacement selection; header ownership hidden by a child pointer | M/H/H |

## High-frequency guardrails

Validated refinements from the [30-function campaign](near-match-30-campaign.md):

| If A → then B | Required evidence | Verified example |
|---|---|---|
| If an extended `Base base; extra;` view emits accesses beyond the allocation while retail writes inside Base, recover those fields in Base's unknown range and remove the extension | Multiple retail accesses prove width/offset; preserve sizeof and audit all consumers | AnimPdata landing_start/end at +0xF8/+0xFC, not +0x104/+0x108 |
| If a validated pointer's only CFG difference is equality-success versus inequality-failure branching, factor the repeated check into a typed inline accessor that returns the pointer on success and null on failure | Preserve the null guard before the instance read, its lazy evaluation, and every caller's loads; never add empty success arms or dummy uses | npc_get_latched closed eight >=95% callers; mission and texture latch accessors closed two more |
| If retail extracts one bit and inserts a zero with rlwimi, use the proven byte bitfield; for raw extraction preserve unsigned-byte promotion before shifting | Bit position and storage width agree with neighboring operations; bit names must not invent semantics | npc_xfer / npc_force_state_for_npc; signed-char shifting initially added srawi in scorpion_summon_collide |
| If address-taken scalar or vector locals occupy swapped slots, reorder only those genuine declarations; distinguish them from compiler-generated by-value argument copies | Calls, argument order, lifetimes, and all field accesses remain identical | pz_fighter_smart_flippy and beetle_squashed closed; render_image's argument copies did not respond, so revert |

- Audit every immediate and displacement before accepting an old soft-ceiling
  comment. In the 30-function near-match campaign, high-scoring functions still
  contained a wrong published count and a speedup store to the wrong existing
  field. A matching opcode multiset does not prove matching behavior.
- Recheck every shared-helper consumer, including those below the selection
  threshold. The NPC latch refinement closed eight functions without losing
  an existing exact match, but changed npc_start_goro_bone_match's coloring
  from 93.11828% to 91.55914%. Report such tradeoffs; do not hide them behind
  the aggregate gain or duplicate helper variants merely to recover a score.
- For `addis scratch, value, H; cmplwi scratch, L`, reconstruct the tested
  32-bit value as `(L - (H << 16)) mod 2^32`. In particular, `H = 0` and
  `L = 0xC602` tests positive `0x0000C602`, not `0xFFFFC602`. Check the actual
  high immediate: m2c's output for `is_pX_airborn` inferred the negative value.
  Keep the source state's proven signed type; adding an unsigned cast can
  remove the retail `addis` even after correcting the constant.
